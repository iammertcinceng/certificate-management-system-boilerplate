...
// Approve a certificate as a partner
// NOTE: This only sets partnerApproved=true, does NOT change status to 'approved'
// Final status change requires: institutionApproved && partnerApproved && adminApproved
export async function POST(req: Request) {
  try {
    ...
    // Partner onaylandığında, eğer institutionApproved zaten true ise
    // ve adminApproved otomatik true ise, status'u 'approved' yap
    const newStatus = certificate.institutionApproved ? 'approved' : 'pending';
    ...
    console.error('POST /api/approvals/approve error', err);
